...
export function menuCloseBackDrop() {
  document.addEventListener("click", function (e) {
    if (
      document.querySelector(".menu-open")?.classList.contains("menu-open") &&
...
export function chengeActiveClassHeaderAnyType() {
  ...
  function chengeActiveClassHeaderClick(e) {
    ...
    if (targetElement.closest(".header__link")) {
      //указать елемент который содержит ссылку в хедере
      ...
      const observer = new IntersectionObserver((entries, observer) => {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (
              "#" + targetId !=
              activeHeaderElement?.getAttribute("data-goto")
            ) {
...
///моя мини валидация с снекбарами об ошибке или удаче
...
/// Модуль для добавления блек дропа но надо что бы были стили и нужная верстка
export function overlay() {
  /// добавить этот код при нажатии на кнопку что бы закрывался и при нажатии на задний фон
  ...
  //сюда добавитьь обычное закртие при нажатии по кнопек
  //     }
